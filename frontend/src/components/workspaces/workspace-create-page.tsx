...
export function WorkspaceCreatePage() {
    ...
    try {
      ...
      if (emailsToInvite.length > 0) {
        await Promise.allSettled(
          emailsToInvite.map(email => createWorkspaceInvitation(workspace.id, { email }))
        );
      }
  ...
  return (
    <>
      ...
      <main className="main-content" style={{ maxWidth: '600px', marginTop: '4rem' }}>
        <section className="workspace-dashboard-card" style={{ padding: '3rem 2rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            ...
            <div className="workspace-panel" style={{ marginTop: '2rem', padding: '1.5rem', background: 'var(--background)', border: '1px solid var(--surface-border)' }}>
              <div className="workspace-form-header">
                <h3 className="workspace-section-title" style={{ fontSize: '1.1rem' }}>Invitar miembros (Opcional)</h3>
                <p className="workspace-section-copy" style={{ marginBottom: '1rem' }}>
                  Añade correos electrónicos de las personas que deseas invitar. Se les enviará una invitación al crear el espacio.
                </p>
              </div>
